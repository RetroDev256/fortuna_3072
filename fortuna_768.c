...
static inline void rand_block(F768State* rng, uint8_t* out);
...
void f768_random_fill(F768State* rng, uint8_t* buf, uint64_t len) {
    if (rng && buf && len) {
...
void f768_add_entropy(F768State* rng, uint8_t* buf, uint64_t len) {
    if (rng && buf && len) {
